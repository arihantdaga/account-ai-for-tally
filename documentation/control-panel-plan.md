# Build plan — Account AI for Tally Control Panel (config server + web UI)
...
```
 ┌──────────────────────────── account-ai-for-tally (single binary) ────────────────────────────┐
 ...
 └────────────────────────────────────────────────────────────────────────────────────┘
             ...
             ▼                                        ▼
   ~/.account-ai-for-tally/config.json   ◄───────────────────────┘
...
| File | Action | Purpose |
|---|---|---|
| `code/src/config.mts` | **new** | Load/save `~/.account-ai-for-tally/config.json`; live connection getter |
...
Config file: `path.join(os.homedir(), '.account-ai-for-tally', 'config.json')`.
...
- Header: "Account AI for Tally — Setup".
...
- `npm test` — existing **78** tests still pass, plus:
  - `config.test.mts`: precedence (file > env > default), `saveConfig` round‑trip, corrupt‑file fallback,
    mtime‑cached live getter. Use a temp `HOME`/dir; do not touch the real `~/.account-ai-for-tally`.
