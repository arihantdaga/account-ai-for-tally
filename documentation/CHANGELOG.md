...
* **Setup control panel (dual-mode binary).** Launched with no arguments the binary opens a local web UI (`127.0.0.1:4321`) to set the Tally host/port/company, test the connection, and register itself with **Claude Desktop** or **Claude Code** (one-click, no JSON editing). Launched with `--mcp` it runs the MCP server as before. Config lives in `~/.account-ai-for-tally/config.json` and is read live on every request.
* **delete-master** tool — a generic delete for any master type (ledger, group, stock item, unit, voucher type, godown, stock group, cost category/centre, currency), accepting a list of names for batch deletion. Verified against a live Tally (batch delete returns the correct deleted count).
