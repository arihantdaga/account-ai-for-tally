...
// Desktop. All state lives in ~/.tally-mcp/config.json (see config.mts).
import { spawn } from 'node:child_process';
import http from 'node:http';
import { XMLParser } from 'fast-xml-parser';
import express from 'express';
import { connectClaudeDesktop } from './claude-desktop.mjs';
import {
  getConfigPath,
  getTallyConnection,
  loadConfig,
...
export async function startControlPanel(): Promise<void> {
  ...
  const listen = (port: number, attemptsLeft: number): void => {
    ...
    server.listen(port, host, () => {
      ...
      console.log(`Tally MCP control panel running at ${url}`);
...
<body>
  <div class="card">
    ...
    <div class="buttons">
      ...
      <button id="connect" type="button" class="primary">Connect to Claude Desktop</button>
    </div>
...
<script>
  ...
  $("connect").onclick = async () => {
    setMsg("Registering with Claude Desktop…");
    try {
      // Save current settings first so Claude uses them.
      await fetch("/api/config", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body()),
      });
      const r = await fetch("/api/connect-claude", { method: "POST" });
      const d = await r.json();
      if (d.ok) {
        setMsg("✅ Added. Restart Claude Desktop, then ask it about your Tally data.\nConfig: " + d.path, "ok");
      } else {
        setMsg("❌ " + (d.error || "Failed to update Claude config"), "err");
      }
    } catch (e) { setMsg("❌ " + e.message, "err"); }
  };
