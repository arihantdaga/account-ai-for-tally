  ...
  return new Promise<string>(async (resolve, reject) => {
    try {
      ...
      for (const col of columns) {
        let sqlDataType = '';
        if (col.datatype === 'number') sqlDataType = 'DECIMAL(18,4)';
        else if (col.datatype === 'boolean') sqlDataType = 'BOOLEAN';
        else if (col.datatype === 'date') sqlDataType = 'DATE';
        else sqlDataType = 'TEXT';
      ...
      setTimeout(
        async () => await conn.run(`DROP TABLE IF EXISTS ${tableId};`),
        15 * 60 * 1000,
      );
  ...
  return new Promise<string>(async (resolve, reject) => {
    try {
      ...
      for (let r = 0; r < lstData.length; r++) {
        const lstRowValue: string[] = [];
        for (let c = 0; c < lstHeader.length; c++) {
          const dataType = lstDataType[c];
          const cellValue = lstData[r][c]?.valueOf();
