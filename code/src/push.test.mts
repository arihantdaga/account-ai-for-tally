...
describe('buildDeleteMasterXML (via handlePush)', () => {
  it('deletes a single master using the uppercased collection tag', async () => {
    const xml = await callAndCapture('delete-master', { collection: 'ledger', name: ['Cash'] }, DELETE_RESPONSE);
    expect(xml).toContain('<LEDGER NAME="Cash" ACTION="Delete"><NAME>Cash</NAME></LEDGER>');
    expect(xml).toContain('<REPORTNAME>All Masters</REPORTNAME>');
  });

  it('maps collection names to the correct Tally tags', async () => {
    const si = await callAndCapture('delete-master', { collection: 'stockitem', name: ['Widget'] }, DELETE_RESPONSE);
    expect(si).toContain('<STOCKITEM NAME="Widget" ACTION="Delete">');
    const vt = await callAndCapture('delete-master', { collection: 'vouchertype', name: ['My VT'] }, DELETE_RESPONSE);
    expect(vt).toContain('<VOUCHERTYPE NAME="My VT" ACTION="Delete">');
  });

  it('deletes multiple masters in one request (batch)', async () => {
    const xml = await callAndCapture('delete-master', { collection: 'ledger', name: ['A', 'B', 'C'] }, DELETE_RESPONSE);
    expect(xml).toContain('<LEDGER NAME="A" ACTION="Delete">');
    expect(xml).toContain('<LEDGER NAME="B" ACTION="Delete">');
    expect(xml).toContain('<LEDGER NAME="C" ACTION="Delete">');
  });

  it('HTML-escapes master names', async () => {
    const xml = await callAndCapture('delete-master', { collection: 'ledger', name: ['A & B'] }, DELETE_RESPONSE);
    expect(xml).toContain('NAME="A &amp; B"');
    expect(xml).toContain('<NAME>A &amp; B</NAME>');
  });

  it('returns an error for an unsupported collection', async () => {
    const result = await handlePush('delete-master', { collection: 'widgets', name: ['X'] });
    expect(result.success).toBe(false);
    expect(result.error).toContain('Unsupported collection');
  });

  it('returns an error when no names are given', async () => {
    const result = await handlePush('delete-master', { collection: 'ledger', name: [] });
    expect(result.success).toBe(false);
