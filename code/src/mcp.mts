...
export async function registerMcpServer(): Promise<McpServer> {
  const mcpServer = new McpServer({
    name: 'Account AI for Tally',
    title: 'Account AI for Tally',
    version: '1.0.0',
  });
  ...
  mcpServer.registerTool(
    'delete-master',
    {
      title: 'Delete Master',
      description:
        'Delete one or more master records of a single type from Tally — a generic delete for ledgers, groups, stock items, units, voucher types, godowns, stock groups, cost categories/centres, or currencies. A master can only be deleted if it has no dependent transactions or child masters. Validate exact names first with list-master (same collection). Destructive — deletions cannot be undone.',
      inputSchema: {
        targetCompany: z.string().optional().describe('optional company name'),
        collection: z
          .enum([
          ...
          .describe('type of master to delete'),
        name: z
          .array(z.string())
          .min(1)
          .describe('exact name(s) of the master(s) to delete'),
      },
      annotations: {
        readOnlyHint: false,
        destructiveHint: true,
    ...
    async (args) => {
      const resp = await handlePush('delete-master', args);
      if (!resp.success) {
        return {
          isError: true,
          content: [
            { type: 'text', text: resp.error || 'Failed to delete master(s)' },
          ],
        };
      }
      return { content: [{ type: 'text', text: JSON.stringify(resp) }] };
